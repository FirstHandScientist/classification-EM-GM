...
def save_image(true_dist, fake_dist, img_dir, step):
    """
    Generates and saves a plot of the true distribution, the generator, and the
    critic.
    """
    true_dist = true_dist.squeeze()
    fake_dist = fake_dist.squeeze()
    fake_dist = fake_dist.cpu().data.numpy()
    true_dist = true_dist.cpu().data.numpy()
    plt.clf()
    plt.scatter(true_dist[:, 0], true_dist[:, 1], c='orange', marker='+')
    plt.scatter(fake_dist[:, 0], fake_dist[:, 1], c='green', marker='+')
    plt.savefig(img_dir + '/frame' + '_' + str(step) + '.jpg')
...
def save(global_step, graph, graph_prior, optim, criterion_dict=None, pkg_dir="", is_best=False, max_checkpoints=None):
    ...
    if max_checkpoints is not None:
        ...
        while len(history) > max_checkpoints:
            ...
            if os.path.exists(path):
                os.remove(path)
            history.pop(0)
def new2old(global_step, path, graph, optim, max_checkpoints=5, is_best=False):
    # saving this step's solution
    save(global_step=global_step,
         graph=graph["new"],
         graph_prior=graph["new"].get_prior(),
         optim=optim,
         pkg_dir=path,
         is_best=is_best,
         max_checkpoints=max_checkpoints)
        
    path = os.path.join(path, _file_at_step(global_step))
    loaded_step, graph_prior = load(os.path.basename(path),
                                    graph=graph["old"], optim=None,
                                    criterion_dict=None,
...
def load(step_or_path, graph, optim=None, criterion_dict=None, pkg_dir="", device=None):
    ...
    try:
        ...
        graph.update_prior(prior)
    except AttributeError:
        graph.get_component().set_actnorm_init(inited=True)
        graph.update_prior(prior)
        for i in range(prior.size(0)):
            graph.source_list[i].inited=True

    print("[Checkpoint]: Load {} successfully".format(save_path))
    return (global_step, prior)
