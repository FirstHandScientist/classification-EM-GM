...
class Gamma:
    @staticmethod
    def likelihood(alpha, beta, x, device):
        #assert type(alpha) == int, "alpha need to be integer at this stage"
        assert beta>0, "beta has to be possitive"
        lnL = alpha * torch.log(beta) + (alpha - 1) * torch.log(x) - beta * x - torch.log( torch.FloatTensor([np.math.factorial(int(alpha))]) ).to(device)
        return lnL
    
    @staticmethod
    def logp(alpha, beta, x, device):
        likelihood = Gamma.likelihood(alpha, beta, x, device)
        return thops.sum(likelihood, dim=[1, 2, 3])
