...
class Trainer(object):
    def __init__(self, graph, graph_prior, optim, lrschedule, loaded_step,
        ...
        self.log_dir = os.path.join(hparams.Dir.log_root, "class{}".format(label) )
        ...
        for epoch in range(self.n_epoches):
            ...
            for i_batch, batch in enumerate(progress):
                ...
                if self.global_step == 0 and self.warm_start is False:
                    # peek that training date for sanity check
                    vutils.save_image(x.data.cpu(), os.path.join(self.log_dir, "peek.png"))
                ...
                if self.naive:
                    ...
                    loss_generative = (torch.mean(torch.sum( torch.exp(-nlog_gamma) * nlog_joint_prob, dim=0)) + torch.mean(nlogdet))/pixel
                    loss_std = 0
                    if self.regulate_std:
                        loss_std = -reg_prior_logp.mean()
                    loss = loss_generative + loss_std
                    if loss < 0:
                        print(nlog_joint_prob)
